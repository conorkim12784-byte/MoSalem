...
async def get_num_for_user_and_group(m: Message):
    ...
    if get_db_checkuserall() is None:
        ...
        for per in get_db_checkuserall():
            user = user + 1
    await m.reply_video(video="https://t.me/OpsTALASHNY/6", caption="""
    ⌔مرحبا بك عزيزي المطور
يمكنك الان معرفه احصائياتي🔔
    """, reply_to_message_id=m.message_id, reply_markup=InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(f"⌔ عدد الاعضاء -> {user}", url="https://t.me/Ops_TALASHNY")
            ],
            [
                InlineKeyboardButton(f"⌔ عدد الجروبات -> {group}", url="https://t.me/Ops_TALASHNY")
            ]
        ])
    )
...
async def get_version_source(m: Message):
    ...
    await m.reply_text(f"⌔ اصدار سورس TALASHNY \n⌔ الاصدار -> {version}\n√", reply_to_message_id=m.message_id)
