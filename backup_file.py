...
async def get_backup(c: Client, m: Message):
    await c.send_document(m.chat.id, "TALASHNY.db", reply_to_message_id=m.message_id)


async def get_backup2(c: Client, m: Message):
    await c.send_document(m.chat.id, "TALASHNY2.db", reply_to_message_id=m.message_id)
